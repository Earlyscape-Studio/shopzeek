...
import { MapPin, User, ShoppingBag, AlertCircle, Package } from "lucide-react";
...
export default async function ProfilePage() {
    ...
    const [{ data: profile }, { data: orders }, { data: addresses }] = await Promise.all([
        ...
        supabase
            ...
            .select(`
                ...
                delivery_address, delivery_date, tracking_url,
                order_items (
    ...
    // Derive billing/delivery info from the most recent order that has an
    // address — used as a fallback when the `addresses` table is empty.
    const lastOrderWithAddress = orders?.find(
        (o: any) => o.delivery_address && String(o.delivery_address).trim()
    ) as any | undefined

    let derivedAddress: {
        name: string; street: string; lgaCity: string; state: string; phone: string;
    } | null = null

    if (lastOrderWithAddress?.delivery_address) {
        const lines: string[] = (lastOrderWithAddress.delivery_address as string)
            .split("\n")
            .map((l: string) => l.trim())
            .filter(Boolean)

        derivedAddress = {
            name:    lines[0] ?? lastOrderWithAddress.customer_name ?? "",
            street:  lines[1] ?? "",
            lgaCity: lines[2] ?? "",
            state:   lines[3] ?? "",
            phone:   lastOrderWithAddress.customer_phone ?? "",
        }
    }

    const hasNoSavedAddresses = !addresses || addresses.length === 0
                ...
                <div className="space-y-6">
                    ...
                    <Card className="shadow-sm border-gray-200">
                        ...
                        <CardContent className="pt-4">
                            {/* Case 1: addresses table has rows (future-proof if table gets fixed) */}
                            {!hasNoSavedAddresses && addresses!.map((addr: any) => (
                                <div
                                    key={addr.id}
                                    className={`rounded-lg border p-3 text-sm mb-3 last:mb-0 ${
                                        addr.is_default
                            ...
                            {/* Case 2: addresses table empty but we have order delivery info */}
                            {hasNoSavedAddresses && derivedAddress && (
                                <div className="rounded-lg border border-orange-200 bg-orange-50 p-3 text-sm">
                                    <span className="inline-block text-[10px] font-bold uppercase tracking-wider text-[#FF5A00] bg-orange-100 px-2 py-0.5 rounded-full mb-2">
                                        Last used
                                    </span>
                                    <p className="font-semibold text-gray-900">{derivedAddress.name}</p>
                                    <p className="text-gray-500 leading-relaxed mt-1 text-xs">
                                        {derivedAddress.street && <>{derivedAddress.street}<br /></>}
                                        {derivedAddress.lgaCity && <>{derivedAddress.lgaCity}<br /></>}
                                        {derivedAddress.state && <>{derivedAddress.state}, Nigeria</>}
                                    </p>
                                    {derivedAddress.phone && (
                                        <p className="text-gray-500 text-xs mt-1">{derivedAddress.phone}</p>
                                    )}
                                </div>
                            )}

                            {/* Case 3: no orders at all yet */}
                            {hasNoSavedAddresses && !derivedAddress && (
...
function OrderStatusBadge({ status }: { status: string }) {
    ...
    return (
        <span className={`text-xs font-semibold px-3 py-1 rounded-full border ${styles[status] ?? "bg-gray-50 text-gray-600 border-gray-200"}`}>
