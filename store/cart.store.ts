...
export const useCartStore = create<CartStore>()(
  persist(
    (set, get) => ({
      ...
      syncWithDB: async () => {
        ...
        const { data } = await supabase
          ...
          .select(
            `
                        quantity,
                        product_id,
                        products ( name, price, deal_price, deal_ends_at, is_deal_active, image_urls, slug )
                    `
          )
        ...
        for (const localItem of localItems) {
          ...
          if (existingDbItem) {
            ...
            mergedMap.set(localItem.product_id, {
              ...existingDbItem,
      ...
      resetForSignout: () =>
        set({ items: [], coupon: null, lastAuthUserId: null }),
